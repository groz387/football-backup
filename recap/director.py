...
SYSTEM_PROMPT = (
    ...
    "Swearing and cultural spice belong ONLY on the first line (hook) and the "
    "last line (close). Analysis cards stay clean evidence."
)
...
# Backup recap primitives plus the crisp mosaic. Viral slam/radar/spiral
# families are gone — they fought the pitch cards and glitched in assembly.
SHAPE_FAMILY = {
    "shot_map": "pitch",
    "goalmouth": "pitch",
    "goal_chain": "hero",
...
ANGLE_VIZ = {
    "upset": ["shot_map", "sterile_domination", "touch_heatmap", "goal_timeline"],
    "robbery": ["shot_map", "sterile_domination", "goal_chain", "momentum"],
    "siege": ["zone_control", "momentum", "shot_map", "goal_chain"],
    "blowout": ["goal_timeline", "shot_map", "goal_chain", "sterile_domination"],
    "comeback": ["momentum", "shot_map", "goal_chain", "zone_control"],
    "stalemate": ["shot_map", "sterile_domination", "zone_control", "momentum"],
    "two_halves": ["momentum", "zone_control", "shot_map", "goal_chain"],
    "keeper": ["goalmouth", "momentum", "sterile_domination", "goal_chain"],
}
...
def visualization_candidates(bundle: MatchBundle, audit: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    candidates = [
        ...
        },
        {
            "id": "standard_stats",
            "title": "Match Numbers",
            "available": total_shots >= 1 or pass_gap >= 1,
            "score": 40 + min(12, shot_gap + on_target_gap + pass_gap * 0.2),
            "reason": "Audited counts side by side when the picture is the numbers themselves.",
            "best_for": "Flashscore fallbacks and matches without tracking coordinates.",
            "avoid_when": "The pitch cards already prove the same counts.",
        },
    ]
...
def _visual_copy(bundle: MatchBundle, audit: dict[str, Any], viz_id: str) -> dict[str, str]:
    ...
    if viz_id == "standard_stats":
        title, insight = _stats_copy(bundle, audit)
        return {
            "kicker": "THE BASELINE",
            "title": title,
            "subtitle": "",
            ...
            "narration": (
                f"{bundle.home} {home.get('shots', 0)} shots to {away.get('shots', 0)}, "
                f"{home.get('shots_on_target', 0)} on target against {away.get('shots_on_target', 0)}."
            ),
        }
    ...
    def write_script(
        ...
        payload = {
            ...
            "rules": [
                ...
                "Profanity, insults and cultural spice belong ONLY on hook_claim, hook_punch and close. "
                "Every analysis narration is one clean evidence sentence: names, minutes, audited counts.",
                language_rule,
            ],
            "language": lang,
            "editor_note": instruction,
