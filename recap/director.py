...
def _visual_copy(bundle: MatchBundle, audit: dict[str, Any], viz_id: str) -> dict[str, str]:
    ...
    if viz_id == "player_spike":
        ...
        return {
            ...
            "insight": (
                f"{spike.get('player') or surname} had {count} {action} "
                f"against {rest} from the rest of the team."
            ),
            "narration": (
                f"{spike.get('player') or surname} put {count} {action} on the tape, "
                f"the rest of the team had {rest}."
            ),
        }
...
def _closing_copy(bundle: MatchBundle, audit: dict[str, Any], hook: dict[str, Any] | None = None) -> dict[str, str]:
    ...
    if winner and loser:
        ...
        insight = bait or f"{winner} took the result. The numbers tell you how."
        ...
        insight = bait or "Nothing on the board. Plenty on the tape."
    else:
        narration = f"It finishes {score.display}. A point each."
        insight = bait or "Level on the board. Split on the map."
