...
def match_design(home: str, away: str) -> dict[str, Any]:
    ...
    Shirt fills come from ``recap.colors`` (auto kits + clash swap). The canvas
    stays pitch-black — team colour belongs to the data, not the background.
    """
    ...
    home_id = dict(_team_identity_cached(str(home), get_team_kind()))
    away_id = dict(_team_identity_cached(str(away), get_team_kind()))
    home_id["fill"] = pair.home.fill
    home_id["primary"] = pair.home.fill
    home_id["secondary"] = pair.home.secondary
    away_id["fill"] = pair.away.fill
    away_id["primary"] = pair.away.fill
    away_id["secondary"] = pair.away.secondary
    home_id["chart"] = readable_on(pair.home.fill)
    away_id["chart"] = readable_on(pair.away.fill)
