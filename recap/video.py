        ...
        quantized.append(
            {
                **scene,
                "frames": frames,
                "clip": round(clip, 6),
                "on_screen": round(clip - timing.TRANSITION, 6),
            }
        )
    return quantized
    ...
    *,
    ...
    for index, scene in enumerate(scene_list, 1):
        ...
        for number in range(frames):
            ...
            if hold_frame is not None and position >= HOLD_AT:
...
def _crossfade_filter(scene_list: list[dict[str, Any]], fps: int) -> tuple[str, str]:
    """Build the xfade chain and return (filter_graph, output_label).

    The nth dissolve starts at ``sum(previous clips) - n * transition`` so that
    the finished video is exactly ``sum(clips) - (n-1) * transition`` long.
    """
    parts = [
    ...
    for index in range(1, len(scene_list)):
        elapsed += float(scene_list[index - 1]["clip"])
        offset = max(0.0, elapsed - index * timing.TRANSITION)
        label = f"x{index}"
        parts.append(
...
def assemble(
    ...
    *,
    ...
    if crossfade and len(scene_list) > 1:
        graph, label = _crossfade_filter(scene_list, fps)
    else:
        graph = ";".join(
            f"[{index}:v]settb=AVTB,fps={fps},format=yuv420p[v{index}]"
            for index in range(len(scene_list))
