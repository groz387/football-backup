...
def visualizations(payload: dict = Body(default_factory=dict)) -> dict:
    try:
        return studio_api.visualization_options(
            str(payload.get("match_dir") or ""),
            int(payload.get("count") or 4),
        )
